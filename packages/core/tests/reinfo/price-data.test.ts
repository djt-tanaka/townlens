...
function createMockClient(fetchTradesFn: (...args: any[]) => any) {
  return {
    fetchTrades: fetchTradesFn,
  } as any;
}
...
describe("buildPriceData", () => {
  it("各都市の取引データを取得して統計を計算する", async () => {
    ...
    const mockClient = createMockClient(fetchTrades);
  ...
  it("マンション以外の取引は除外される", async () => {
    const fetchTrades = vi.fn().mockResolvedValueOnce([
  ...
  it("quarterパラメータを渡せる", async () => {
    ...
    expect(fetchTrades).toHaveBeenCalledWith(
