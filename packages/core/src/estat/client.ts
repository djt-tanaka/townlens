...
export class EstatApiClient {
  ...
  private async request(endpoint: string, params: Record<string, string | number | undefined>): Promise<any> {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        // status=2+: APIエラー
        throw new AppError(`e-Stat APIエラー(status=${status})`, errorMsg ? [errorMsg] : []);
      } catch (error) {
    ...
    if (axios.isAxiosError(lastError)) {
      throw new AppError("e-Stat APIへの通信に失敗しました", [
  ...
  async getMetaInfo(statsDataId: string): Promise<any> {
    if (this.cache) {
      const cached = await this.cache.get<any>(`estat:meta:${statsDataId}`);
      if (cached) {
        return cached;
      }
    }
    ...
    const result = data?.GET_META_INFO?.METADATA_INF ?? data;
